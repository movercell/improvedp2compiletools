...
BEGIN_BYTESWAP_DATADESC( CDispMultiBlend )
	...
	DEFINE_ARRAY( m_vMultiBlendColors, FIELD_VECTOR, MAX_MULTIBLEND_CHANNELS ),
END_BYTESWAP_DATADESC()
...
template< class T > static void AddLump( int lumpnum, T *pData, int count, int version = 0, int alignment = 1 );
template< class T > static void AddLump( int lumpnum, CUtlVector<T> &data, int version = 0, int alignment = 1 );
...
static void AddOcclusionLump( )
{
	...
	V_memset( lump->fourCC, 0, sizeof( lump->fourCC ) );
...
static void AddLumpInternal( int lumpnum, void *data, int len, int version, int alignment )
{
	lump_t *lump;
...
//-----------------------------------------------------------------------------
template< class T >
static void AddLump( int lumpnum, T *pData, int count, int version, int alignment )
{
	AddLumpInternal( lumpnum, pData, count * sizeof(T), version, alignment );
}

template< class T >
static void AddLump( int lumpnum, CUtlVector<T> &data, int version, int alignment )
{
	AddLumpInternal( lumpnum, data.Base(), data.Count() * sizeof(T), version, alignment );
}
...
//		ParseEntities ();
	...
	totalmemory += GlobUsage( "entdata",		dentdata.Count(), 0 );	// goal is <384K
	...
	totalmemory += ArrayUsage( "Prop Hull Verts", g_PropHullVerts.Count(), 0, sizeof( g_PropHullVerts[0] ) );
	totalmemory += GlobUsage( "Prop Hull Blob", g_PropBlob.Count(), 0 );
	totalmemory += ArrayUsage( "Prop Hull Lists", g_PropCollision.Count(), 0, sizeof( g_PropCollision[0] ) );
	totalmemory += ArrayUsage( "Prop Hulls", g_PropHulls.Count(), 0, sizeof( g_PropHulls[0] ) );
	totalmemory += ArrayUsage( "Prop Hull trilists", g_PropTris.Count(), 0, sizeof( g_PropTris[0] ) );
...
bool CToolBSPTree::EnumerateLeavesInSphereWithFlagSet( const Vector &center, float radius, ISpatialLeafEnumerator *pEnum, int context, int nFlagsCheck )
{
...
{
public:
	CLeafMakeList( unsigned short *pList, int listMax ) :
		pList( pList ),
		listMax( listMax ),
		index( 0 )
	{
	...
	bool EnumerateLeaf( int leaf, int context ) override
	{
		if ( index < listMax )
		{
			pList[index++] = leaf;
...
{
	CLeafMakeList listEnum( pList, listMax );
	EnumerateLeavesInBox( mins, maxs, &listEnum, 0 );
...
int CToolBSPTree::ListLeavesInSphereWithFlagSet( int *pLeafsInSphere, const Vector &vecCenter, float flRadius, int nLeafCount, const uint16 *pLeafs, int nLeafStride, int nFlagsCheck )
{
	...
	{
		dleaf_t &leaf = dleafs[*pLeafs];
		VECTOR_COPY( leaf.mins, mins );
		VECTOR_COPY( leaf.maxs, maxs );

		if ( IsBoxIntersectingSphere( mins, maxs, vecCenter, flRadius ) )
		{
			pLeafsInSphere[index++] = i;
		}

		pLeafs = ( const uint16 * )( uintptr_t( pLeafs ) + nLeafStride );
	}
	return index;
}
...
*/
	...
	V_memset( pOutBSPHeader->lumps[LUMP_GAME_LUMP].fourCC, 0, sizeof( pOutBSPHeader->lumps[LUMP_GAME_LUMP].fourCC ) );
...
bool RepackBSP( CUtlBuffer &inputBuffer, CUtlBuffer &outputBuffer, CompressFunc_t pCompressFunc )
{
	...
	BSPHeader_t sOutBSPHeader = *pInBSPHeader;
	...
	for ( int i = 0; i < HEADER_LUMPS; ++i )
	{
		...
		V_memset( sOutBSPHeader.lumps[lumpNum].fourCC, 0, sizeof( sOutBSPHeader.lumps[lumpNum].fourCC ) );
...
bool SwapBSPFile( const char *pInFilename, const char *pOutFilename, bool bSwapOnLoad, VTFConvertFunc_t pVTFConvertFunc, VHVFixupFunc_t pVHVFixupFunc, CompressFunc_t pCompressFunc )
{
	...
	if ( pCompressFunc )
	{
		...
		if ( !RepackBSP( inputBuffer, outputBuffer, pCompressFunc ) )
		{
